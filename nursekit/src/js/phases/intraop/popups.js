...
(function () {
  ...
  function ensureCss() {
    ...
    s.textContent = [
      /* ── Kart konteyner (preop .obj-popup paritesi) ── */
      '#obj-popup.ipv2-host{width:min(380px,calc(100vw - 32px))!important;max-width:380px!important;overflow:hidden!important;padding:0!important}',
      '.ipv2,.ipv2 *{box-sizing:border-box;max-width:100%}',
      '.ipv2{font-family:inherit;color:var(--ink,#e8eef5);background:var(--panel,#0f2138);border:0;border-radius:14px;width:100%;min-width:0;max-height:min(68vh,560px);display:flex;flex-direction:column;overflow:hidden;font-size:12.5px;line-height:1.5}',
      '.ipv2.hs-true{box-shadow:inset 0 0 0 1px rgba(217,99,113,.45)}',
      '.ipv2.hs-cond{box-shadow:inset 0 0 0 1px rgba(224,165,88,.40)}',
      '.ipv2.breach{box-shadow:inset 0 0 0 2px rgba(217,99,113,.55)}',

      /* ── Header (preop .otitle-row paritesi) ── */
      '.ipv2-h{padding:12px 14px 8px;position:relative;background:transparent;border:0}',
      '.ipv2-cls{position:absolute;top:8px;right:11px;cursor:pointer;color:var(--ink-mute,#a4b4c7);font-size:18px;font-weight:600;line-height:1;user-select:none;background:transparent;border:0;padding:2px 6px;border-radius:6px}',
      '.ipv2-cls:hover{color:var(--ink,#e8eef5);background:rgba(92,196,214,.10)}',
      '.ipv2-t{font-size:13px;line-height:1.25;letter-spacing:.4px;color:var(--teal,#5cc4d6);font-weight:800;padding-right:22px;text-transform:uppercase;margin:0}',
      '.ipv2-st{margin-top:4px;color:var(--ink-mute,#a4b4c7);font-size:11.5px;line-height:1.4;font-weight:500}',
      '.ipv2-d{margin-top:6px;color:var(--ink,#e8eef5);font-size:12px;line-height:1.5;font-weight:400;overflow-wrap:anywhere}',
      '.ipv2-chips{display:flex;flex-wrap:wrap;gap:5px;margin-top:8px;align-items:center}',
      /* Chip stili — preop .scene-risk-chip paritesi */
      '.ipv2-chip{display:inline-flex;align-items:center;gap:4px;padding:3px 8px;border-radius:999px;background:rgba(92,196,214,.10);border:1px solid rgba(92,196,214,.32);color:var(--teal,#5cc4d6);font-size:10px;font-weight:700;letter-spacing:.2px;line-height:1.2;white-space:nowrap;text-transform:uppercase}',
      '.ipv2-chip.gckl{background:rgba(76,184,138,.10);border-color:rgba(76,184,138,.42);color:var(--green,#4cb88a)}',
      '.ipv2-chip.hs{background:rgba(217,99,113,.10);border-color:rgba(217,99,113,.45);color:#f0a0aa}',
      '.ipv2-chip.s-pending,.ipv2-chip.s-partial{background:rgba(224,165,88,.10);border-color:rgba(224,165,88,.42);color:var(--amber,#e0a558)}',
      '.ipv2-chip.s-complete{background:rgba(76,184,138,.10);border-color:rgba(76,184,138,.45);color:var(--green,#4cb88a)}',
      '.ipv2-chip.s-wrong,.ipv2-chip.s-breach{background:rgba(217,99,113,.10);border-color:rgba(217,99,113,.45);color:#f0a0aa}',
      '.ipv2-chip.score{background:rgba(76,184,138,.10);border-color:rgba(76,184,138,.42);color:var(--green,#4cb88a)}',
      ...
      /* ── Body & scroll ── */
      '.ipv2-b{padding:6px 14px 14px;overflow-y:auto;overflow-x:hidden;overscroll-behavior:contain;flex:1;scrollbar-width:thin;scrollbar-color:rgba(92,196,214,.40) rgba(255,255,255,.04)}',
      '.ipv2-b::-webkit-scrollbar{width:8px}',
      '.ipv2-b::-webkit-scrollbar-track{background:rgba(255,255,255,.03);border-radius:999px}',
      '.ipv2-b::-webkit-scrollbar-thumb{background:rgba(92,196,214,.38);border-radius:999px;border:2px solid var(--panel,#0f2138)}',
      '.ipv2-b::-webkit-scrollbar-thumb:hover{background:rgba(92,196,214,.55)}',

      /* ── Progress bar (preop GCKL bar paritesi) ── */
      '.ipv2-prog{display:flex;align-items:center;gap:8px;margin:4px 0 10px}',
      '.ipv2-prog-bar{flex:1;height:6px;background:rgba(255,255,255,.06);border-radius:999px;overflow:hidden}',
      '.ipv2-prog-fill{height:100%;background:linear-gradient(90deg,var(--teal-2,#3aa6b9),var(--teal,#5cc4d6));transition:width .25s;border-radius:999px}',
      '.ipv2-prog-txt{font-size:10.5px;color:var(--ink-mute,#a4b4c7);font-weight:700;min-width:44px;text-align:right;white-space:nowrap;font-family:var(--font-mono,monospace)}',

      /* ── Bölüm başlığı (preop note-title paritesi) ── */
      '.ipv2-sec{margin:10px 0 0}',
      '.ipv2-sec-t{font-size:10px;text-transform:uppercase;letter-spacing:.5px;color:var(--ink-mute,#a4b4c7);font-weight:700;margin-bottom:5px}',

      /* ── Section box dili (preop .scene-note-card paritesi) ── */
      '.ipv2-role,.ipv2-link,.ipv2-warn,.ipv2-fail,.ipv2-succ,.ipv2-prereq,.ipv2-evid{padding:9px 11px;border-radius:10px;font-size:12px;line-height:1.5;background:rgba(92,196,214,.05);border:1px solid rgba(92,196,214,.20);color:var(--ink,#e8eef5);overflow-wrap:anywhere}',
      '.ipv2-link{border-style:dashed;color:var(--ink,#cfe2ec)}',

      /* ── KRİTİK HATA bölümü ── */
      '.ipv2-warn{border-color:rgba(224,165,88,.45);background:rgba(224,165,88,.08);color:#f1e1b8}',
      '.ipv2-warn.hs{border-color:rgba(217,99,113,.55);background:rgba(217,99,113,.10);color:#f7c8c8}',
      '.ipv2-warn b{display:block;margin-bottom:3px;font-size:10px;letter-spacing:.5px;text-transform:uppercase;color:inherit;font-weight:700;opacity:.85}',
      '.ipv2-fail{margin-top:6px;border-color:rgba(217,99,113,.40);background:rgba(217,99,113,.07);color:#eebabf}',
      '.ipv2-succ{margin-top:6px;border-color:rgba(76,184,138,.45);background:rgba(76,184,138,.07);color:var(--green,#4cb88a)}',
      '.ipv2-fail b,.ipv2-succ b{display:inline;margin-right:4px;font-size:11px;text-transform:none;letter-spacing:0;font-weight:700}',

      /* ── Prerequisite ── */
      '.ipv2-prereq{border-style:dashed;border-color:rgba(224,165,88,.42);background:rgba(224,165,88,.08);color:var(--amber,#e0a558);margin-bottom:8px;font-size:11.5px}',
      '.ipv2-prereq.ok{border-color:rgba(76,184,138,.42);background:rgba(76,184,138,.08);color:var(--green,#4cb88a)}',

      /* ── Klinik not / kanıt ── */
      '.ipv2-evid{margin-top:8px;background:rgba(92,196,214,.06);border-color:rgba(92,196,214,.22);color:var(--ink,#cfe2ec)}',
      '.ipv2-evid b{color:var(--teal,#5cc4d6);display:block;font-size:10px;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;font-weight:700}',
      '.ipv2-evid div{margin-top:3px;font-size:11.5px;line-height:1.5}',
      '.ipv2-evid div b{display:inline;font-size:11.5px;text-transform:none;letter-spacing:0;margin:0 4px 0 0;color:var(--ink,#e8eef5);opacity:.85}',

      /* ── Butonlar (preop .scene-action-btn paritesi) ── */
      '.ipv2-actions{display:grid;grid-template-columns:1fr;gap:7px;margin-top:6px}',
      '.ipv2-btn{display:flex;align-items:center;width:100%;min-height:38px;padding:9px 12px;background:rgba(92,196,214,.10);border:1px solid var(--teal-2,#3aa6b9);border-radius:10px;color:var(--teal,#5cc4d6);cursor:pointer;font-size:12px;font-weight:600;text-align:left;line-height:1.35;transition:background .15s ease,color .15s ease,transform .15s ease;font-family:inherit;letter-spacing:.1px;white-space:normal;overflow-wrap:break-word;hyphens:auto}',
      '.ipv2-btn:hover:not([disabled]){background:var(--teal,#5cc4d6);color:#062533}',
      '.ipv2-btn[disabled]{cursor:not-allowed;opacity:.55}',
      '.ipv2-btn.primary{border-color:rgba(217,99,113,.55);background:rgba(217,99,113,.12);color:#ffd7df}',
      '.ipv2-btn.primary:hover:not([disabled]){background:var(--rose,#d96371);color:#fff}',
      '.ipv2-btn.done{border-color:rgba(76,184,138,.45);background:rgba(76,184,138,.10);color:var(--green,#4cb88a);cursor:default}',
      '.ipv2-btn.done:hover{background:rgba(76,184,138,.10);color:var(--green,#4cb88a);transform:none}',
      '.ipv2-btn.done:before{content:"✓ ";font-weight:800;margin-right:2px;color:var(--green,#4cb88a)}',
      '.ipv2-btn.mcq{border-color:rgba(155,137,196,.55);background:rgba(155,137,196,.10);color:#d1c4e9}',
      '.ipv2-btn.mcq:hover:not([disabled]){background:var(--violet,#9b89c4);color:#fff}',
      '.ipv2-btn.hint{border-color:rgba(204,226,154,.40);background:rgba(204,226,154,.07);color:#cce29a}',
      '.ipv2-btn.hint:hover:not([disabled]){background:rgba(204,226,154,.18);color:#e9f5c8}',
      '.ipv2-btn.stop{background:rgba(217,99,113,.15);border-color:rgba(217,99,113,.55);color:#fbd5d5;font-weight:700;margin-top:8px;text-align:center;justify-content:center}',
      '.ipv2-btn.stop:hover:not([disabled]){background:var(--rose,#d96371);color:#fff}',

      /* ── AI ipucu kutusu ── */
      '.ipv2-ai{font-size:11.5px;margin-top:8px;color:#cce29a;padding:9px 11px;background:rgba(204,226,154,.07);border:1px solid rgba(204,226,154,.32);border-radius:10px;line-height:1.5}',

      /* ── Kontrol Listesi (substep checklist — preop nk-step paritesi, kompakt) ── */
      '.ipv2-step-list{display:grid;gap:4px;margin-top:2px;padding:0;background:transparent;border:0}',
      '.ipv2-step{font-size:11.5px;color:var(--ink-mute,#a4b4c7);display:flex;gap:8px;align-items:flex-start;line-height:1.42;padding:2px 0}',
      '.ipv2-step.done{color:var(--green,#4cb88a)}',
      '.ipv2-step-icon{flex-shrink:0;width:13px;display:inline-flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;line-height:1.4;color:var(--ink-dim,#6f8197)}',
      '.ipv2-step.done .ipv2-step-icon{color:var(--green,#4cb88a)}',
      '.ipv2-step-text{flex:1;min-width:0;overflow-wrap:anywhere;font-weight:500;letter-spacing:.1px}',
      '.ipv2-step.done .ipv2-step-text{font-weight:500;text-decoration:none}',

      /* ── Bağlı Görevler · İlerleme listesi (preop gckl-board task-item paritesi) ── */
      '.ipv2-task-list{display:grid;gap:7px}',
      '.ipv2-task-list .task-item{display:grid;grid-template-columns:24px minmax(0,1fr);gap:10px;padding:9px 11px;border-radius:10px;border:1px solid rgba(92,196,214,.20);background:rgba(92,196,214,.05);align-items:start;line-height:1.4}',
      '.ipv2-task-list .task-item.done{border-color:rgba(76,184,138,.45);background:rgba(76,184,138,.10)}',
      '.ipv2-task-list .task-item.pending{border-color:rgba(224,165,88,.32);background:rgba(224,165,88,.06)}',
      '.ipv2-task-list .task-item.pending.critical-pending{border-color:rgba(217,99,113,.45);background:rgba(217,99,113,.07)}',
      '.ipv2-task-list .check{width:24px;height:24px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;border-radius:6px;line-height:1}',
      '.ipv2-task-list .check.state-done{background:rgba(76,184,138,.20);color:var(--green,#4cb88a)}',
      '.ipv2-task-list .check.state-pending{background:rgba(224,165,88,.20);color:var(--amber,#e0a558)}',
      '.ipv2-task-list .check.state-critical{background:rgba(217,99,113,.20);color:var(--rose,#d96371)}',
      '.ipv2-task-list .task-main{min-width:0}',
      '.ipv2-task-list .task-head{display:flex;flex-direction:column;gap:5px}',
      '.ipv2-task-list .task-head .lbl{font-size:12px;font-weight:600;color:var(--ink,#e8eef5);line-height:1.35}',
      '.ipv2-task-list .task-head .badges{display:flex;flex-wrap:wrap;gap:4px}',
      '.ipv2-task-list .badge{display:inline-flex;align-items:center;padding:2px 7px;border-radius:999px;font-size:9.5px;font-weight:700;text-transform:uppercase;letter-spacing:.4px;line-height:1.3;white-space:nowrap;border:1px solid transparent}',
      '.ipv2-task-list .badge.crit{background:rgba(217,99,113,.14);border-color:rgba(217,99,113,.45);color:var(--rose,#d96371)}',
      '.ipv2-task-list .badge.state-done{background:rgba(76,184,138,.14);border-color:rgba(76,184,138,.45);color:var(--green,#4cb88a)}',
      '.ipv2-task-list .badge.state-critical{background:rgba(217,99,113,.14);border-color:rgba(217,99,113,.45);color:var(--rose,#d96371)}',
      '.ipv2-task-list .badge.free{background:rgba(224,165,88,.14);border-color:rgba(224,165,88,.45);color:var(--amber,#e0a558)}',
      '.ipv2-task-list .badge.gline{background:rgba(92,196,214,.10);border-color:rgba(92,196,214,.40);color:var(--teal,#5cc4d6);font-family:var(--font-mono,monospace);letter-spacing:.2px}',

      /* ── GCKL HARİTALAMA — kompakt tablo gibi grid (preop dilinde) ── */
      '.ipv2-map{font-size:11.5px;color:var(--ink,#cfe2ec);margin-top:10px;padding:11px 12px;background:rgba(92,196,214,.05);border:1px solid rgba(92,196,214,.22);border-radius:10px;line-height:1.5;display:block}',
      '.ipv2-map-t{display:block;font-size:10px;text-transform:uppercase;letter-spacing:.5px;color:var(--teal,#5cc4d6);font-weight:700;margin-bottom:8px;padding-bottom:5px;border-bottom:1px solid rgba(92,196,214,.16)}',
      '.ipv2-map-grid{display:grid;grid-template-columns:minmax(90px,auto) minmax(0,1fr);gap:4px 10px}',
      '.ipv2-map-grid b{color:var(--ink-mute,#a4b4c7);font-size:10px;text-transform:uppercase;letter-spacing:.45px;font-weight:700;align-self:start}',
      '.ipv2-map-grid span{color:var(--ink,#e8eef5);font-size:11.5px;font-weight:500;min-width:0;overflow-wrap:anywhere;word-break:normal;font-family:var(--font-mono,monospace)}',
      '.ipv2-map-grid span.hs-strict{color:#f0a0aa;font-weight:700;font-family:inherit}',
      '.ipv2-map-grid span.hs-cond{color:var(--amber,#e0a558);font-weight:700;font-family:inherit}',
      '.ipv2-map-grid span.hs-none{color:var(--green,#4cb88a);font-weight:600;font-family:inherit}',
      /* ── Mobil / küçük ekran ── */
      '@media (max-width:640px){',
      '  #obj-popup.ipv2-host{width:calc(100vw - 18px)!important;max-width:calc(100vw - 18px)!important}',
      '  .ipv2{width:100%;max-width:none;max-height:min(82dvh,calc(100dvh - 80px));border-radius:14px 14px 0 0;font-size:12px}',
      '  .ipv2-h{padding:12px 14px 8px}',
      '  .ipv2-t{font-size:13px;padding-right:24px}',
      '  .ipv2-b{padding:6px 14px 14px}',
      '  .ipv2-btn{padding:10px 12px;font-size:12px;min-height:40px}',
      '  .ipv2-cls{font-size:20px;top:10px;right:13px}',
      '  .ipv2-map-grid{grid-template-columns:auto 1fr;gap:4px 8px}',
      '}',
      /* ── Mikro Senaryo Modal (ayrı katman, ana karttan bağımsız) ── */
      '#ipv2-mcq-backdrop{position:fixed;inset:0;background:rgba(4,12,22,.72);backdrop-filter:blur(6px);z-index:1200;display:flex;align-items:center;justify-content:center;padding:20px;opacity:0;pointer-events:none;transition:opacity .18s ease}',
      '#ipv2-mcq-backdrop.visible{opacity:1;pointer-events:auto}',
      '.ipv2-mcq-modal{width:min(520px,calc(100vw - 32px));max-height:min(82vh,640px);background:linear-gradient(180deg,rgba(10,24,38,.99),rgba(12,27,42,.99));border:1px solid rgba(168,127,224,.45);border-radius:16px;color:var(--ink,#e8eef2);display:flex;flex-direction:column;box-shadow:0 24px 70px rgba(0,0,0,.55);overflow:hidden;font-size:12.5px;line-height:1.45;transform:translateY(8px) scale(.98);transition:transform .2s ease}',
      '#ipv2-mcq-backdrop.visible .ipv2-mcq-modal{transform:translateY(0) scale(1)}',
      '.ipv2-mcq-modal-h{padding:14px 18px 11px;border-bottom:1px solid rgba(168,127,224,.22);background:linear-gradient(180deg,rgba(40,24,68,.45),rgba(20,12,38,.05));position:relative}',
      '.ipv2-mcq-modal-tag{font-size:9.8px;letter-spacing:.08em;text-transform:uppercase;color:#d4bbf0;font-weight:800}',
      '.ipv2-mcq-modal-title{font-size:15px;line-height:1.25;color:var(--ink,#f0e8ff);font-weight:740;margin-top:3px;padding-right:28px}',
      '.ipv2-mcq-modal-cls{position:absolute;top:11px;right:14px;cursor:pointer;color:var(--ink-mute,#9087a0);font-size:22px;line-height:1;user-select:none;font-weight:600}',
      '.ipv2-mcq-modal-cls:hover{color:var(--ink,#e8eef2)}',
      '.ipv2-mcq-modal-b{padding:14px 18px 16px;overflow-y:auto;flex:1;scrollbar-width:thin;scrollbar-color:rgba(168,127,224,.40) rgba(255,255,255,.05)}',
      '.ipv2-mcq-modal-b::-webkit-scrollbar{width:8px}',
      '.ipv2-mcq-modal-b::-webkit-scrollbar-thumb{background:rgba(168,127,224,.40);border-radius:999px;border:2px solid rgba(10,24,38,.98)}',
      '.ipv2-mcq-modal-scenario{background:rgba(168,127,224,.07);border-left:3px solid rgba(168,127,224,.5);border-radius:0 10px 10px 0;padding:10px 13px;font-size:12.5px;color:var(--ink,#e5dcf2);line-height:1.5;margin-bottom:12px}',
      '.ipv2-mcq-modal-scenario b{display:block;font-size:9.5px;letter-spacing:.08em;text-transform:uppercase;color:#d4bbf0;font-weight:800;margin-bottom:3px}',
      '.ipv2-mcq-modal-q{font-size:13.5px;font-weight:700;color:var(--ink,#f0e8ff);line-height:1.4;margin-bottom:10px}',
      '.ipv2-mcq-modal-opts{display:flex;flex-direction:column;gap:7px}',
      '.ipv2-mcq-modal-opt{display:block;width:100%;padding:10px 13px;background:rgba(255,255,255,.04);border:1px solid rgba(168,127,224,.25);border-radius:10px;color:var(--ink,#d8d0e8);cursor:pointer;font-size:12.5px;text-align:left;font-family:inherit;line-height:1.42;transition:all .15s;white-space:normal;overflow-wrap:break-word}',
      '.ipv2-mcq-modal-opt:hover:not([disabled]){background:rgba(168,127,224,.10);border-color:rgba(168,127,224,.50);transform:translateY(-1px)}',
      '.ipv2-mcq-modal-opt[disabled]{cursor:not-allowed}',
      '.ipv2-mcq-modal-opt.correct{background:rgba(63,196,128,.12);border-color:rgba(63,196,128,.55);color:#a0e6bd}',
      '.ipv2-mcq-modal-opt.wrong{background:rgba(217,99,113,.12);border-color:rgba(217,99,113,.55);color:#f0a0aa}',
      '.ipv2-mcq-modal-fb{margin-top:14px;padding:11px 13px;border-radius:10px;font-size:12.5px;line-height:1.5}',
      '.ipv2-mcq-modal-fb.correct{background:rgba(63,196,128,.09);color:#a8e8c4;border-left:3px solid rgba(63,196,128,.60)}',
      '.ipv2-mcq-modal-fb.wrong{background:rgba(217,99,113,.09);color:#f0aab0;border-left:3px solid rgba(217,99,113,.60)}',
      '.ipv2-mcq-modal-foot{padding:11px 18px 14px;border-top:1px solid rgba(168,127,224,.18);display:flex;justify-content:flex-end;gap:8px}',
      '.ipv2-mcq-modal-foot button{padding:8px 16px;border-radius:9px;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;border:1px solid rgba(168,127,224,.40);background:rgba(168,127,224,.10);color:#d4bbf0;transition:all .15s}',
      '.ipv2-mcq-modal-foot button:hover{background:rgba(168,127,224,.18)}',
      '@media (max-width:640px){',
      '  .ipv2-mcq-modal{width:calc(100vw - 24px);max-height:min(86vh,calc(100dvh - 60px))}',
      '  .ipv2-mcq-modal-h{padding:12px 14px 10px}',
      '  .ipv2-mcq-modal-title{font-size:14px}',
      '  .ipv2-mcq-modal-b{padding:12px 14px 14px}',
      '}'
    ].join('\n');
  ...
  // =================================================================
  //  MİKRO SENARYO MODAL (ayrı katman — ana karttan bağımsız)
  //  Preop paritesi: ana popup bilgi/doğrulama kartıdır; karar sorusu
  //  ayrı bir etkileşim katmanında açılır. Cevap verilince feedback
  //  modal içinde görünür; modal kapanınca ana evidence kartı yerinde
  //  kalır (anchor lastAnchor üzerinden korunur).
  // =================================================================
  function openIntraopMcqModal(card, clinicalKey, obj, nodeId) {
    if (!card || !card.microScenario) return false;
    var sc = card.microScenario;

    // Önceki açık modal varsa kaldır
    var prev = document.getElementById('ipv2-mcq-backdrop');
    ...
    backdrop.id = 'ipv2-mcq-backdrop';

    var h = '';
    h += '<div class="ipv2-mcq-modal" role="dialog" aria-modal="true">';
    h += '<div class="ipv2-mcq-modal-h">';
    h += '<span class="ipv2-mcq-modal-cls" data-mcq-close="1">×</span>';
    h += '<div class="ipv2-mcq-modal-tag">Mikro Karar Senaryosu · ' + esc(card.title) + '</div>';
    h += '<div class="ipv2-mcq-modal-title">' + esc(sc.title || 'Mikro Karar Senaryosu') + '</div>';
    h += '</div>';

    h += '<div class="ipv2-mcq-modal-b">';
    h += '<div class="ipv2-mcq-modal-scenario"><b>Senaryo</b>' + esc(sc.scenarioText) + '</div>';
    h += '<div class="ipv2-mcq-modal-q">' + esc(sc.questionText) + '</div>';
    h += '<div class="ipv2-mcq-modal-opts">';
    (sc.options || []).forEach(function (opt, k) {
      ...
      h += '<button class="ipv2-mcq-modal-opt' + ocls + '" data-mcq-opt="' + k + '"' + (answered ? ' disabled' : '') + '>' + esc(opt) + '</button>';
    });
    h += '</div>';

    if (answered) {
      var ok = (snap.rationaleAnswer === ci);
      h += '<div class="ipv2-mcq-modal-fb ' + (ok ? 'correct' : 'wrong') + '">' +
           esc(ok ? sc.correctFeedback : sc.wrongFeedback) + '</div>';
    }

    h += '</div>'; // body

    h += '<div class="ipv2-mcq-modal-foot">';
    h += '<button data-mcq-close="1">Kapat</button>';
    h += '</div>';
    ...
    // visible class transition için bir sonraki frame
    ...
    backdrop.onclick = function (ev) {
      ...
      // Backdrop kendisine tıklanırsa kapat
      if (t === backdrop) { closeModal(); return; }

      var closeBtn = t.closest && t.closest('[data-mcq-close]');
      if (closeBtn) { closeModal(); return; }

      var optBtn = t.closest && t.closest('[data-mcq-opt]');
      if (!optBtn || optBtn.disabled) return;

      var net = getNet();
      if (!net || !net.answerRationale) { closeModal(); return; }

      var chosen = parseInt(optBtn.dataset.mcqOpt, 10);
      net.answerRationale(nodeId, chosen);
      refreshIntraopGcklViews('mcq:option' + chosen);

      // Modal içinde feedback'i hemen göster — kullanıcı sonucu görsün
      var freshSnap = getNodeSnap(nodeId);
      var isAnswered = freshSnap && typeof freshSnap.rationaleAnswer === 'number';
      var wasOk = isAnswered && (freshSnap.rationaleAnswer === ci);

      // Seçenekleri disable et + correct/wrong işaretle
      var opts = backdrop.querySelectorAll('.ipv2-mcq-modal-opt');
      opts.forEach(function (b, idx) {
        b.disabled = true;
        if (idx === ci) b.classList.add('correct');
        else if (idx === chosen) b.classList.add('wrong');
      });

      // Feedback varsa kaldır (re-open senaryosu), sonra yenisini ekle
      var oldFb = backdrop.querySelector('.ipv2-mcq-modal-fb');
      if (oldFb) oldFb.parentNode.removeChild(oldFb);

      var fb = document.createElement('div');
      fb.className = 'ipv2-mcq-modal-fb ' + (wasOk ? 'correct' : 'wrong');
      fb.textContent = wasOk ? sc.correctFeedback : sc.wrongFeedback;
      var body = backdrop.querySelector('.ipv2-mcq-modal-b');
      if (body) body.appendChild(fb);

      // Ana kartı arka planda re-render et (lastAnchor sayesinde aynı konumda)
      try { renderIntraProtocolCard(card, clinicalKey, obj, null, null); } catch (e) {}
    };
  ...
  // window'a expose (debug + diğer modüller çağırabilsin)
  try { if (typeof window !== 'undefined') window.openIntraopMcqModal = openIntraopMcqModal; } catch (e) {}
  ...
  // Anchor state — popup'ın son tıklanma noktası (viewport koordinatı).
  // Evidence işaretleme sonrası kart yeniden render edildiğinde, x/y null gelse
  // bile popup aynı yerde kalır (preop showObjPopup paritesi).
  var lastAnchor = { x: null, y: null, clinicalKey: null };
  ...
  function renderIntraProtocolCard(card, clinicalKey, obj, x, y) {
    ...
    var mcqCorrect = mcqAnswered && (snap.rationaleAnswer === card.microScenario.correctIndex);
    ...
    if (hasMcq) {
      if (!mcqAnswered) {
        ...
        h += '<button class="ipv2-btn mcq done" data-mcq-toggle="1">' +
             (mcqCorrect ? '✓ Mikro senaryo: doğru cevap (tekrar görüntüle)' : '✗ Mikro senaryo: yanlış cevap (tekrar görüntüle)') +
             '</button>';
      }
    }
    ...
    // MCQ inline KALDIRILDI — Mikro senaryo artık ayrı modal'da açılır
    // (renderIntraopMcqModal). Ana popup yalnızca tetikleyici butonu gösterir.
    ...
    // Evidence Checklist (ek doğrulama listesi) — primary, görünür kalır
    if (requiredEvs.length) {
    ...
    // Bağlı Görevler · İlerleme (preop GCKL board task-item paritesi)
    // İlgili karta bağlı intraop görevleri evidence ilerlemesine göre yeşilleşir.
    var linkedTasks = [];
    try {
      var phase = (window.App && App.currentPatient && App.currentPatient.intraop) || null;
      var allT = phase && Array.isArray(phase.tasks) ? phase.tasks : [];
      linkedTasks = allT.filter(function (t) {
        if (!t) return false;
        if (t.linkedNode === nodeId || t.gcklNode === nodeId) return true;
        // İkincil eşleme: linkedClinicalKey veya linkedObjects içinde bu kart varsa
        if (clinicalKey) {
          if (t.linkedClinicalKey === clinicalKey) return true;
          if (Array.isArray(t.linkedObjects) && t.linkedObjects.indexOf(clinicalKey) >= 0) return true;
        }
        return false;
      });
    } catch (e) { linkedTasks = []; }
    ...
    if (linkedTasks.length) {
      h += '<div class="ipv2-sec"><div class="ipv2-sec-t">Bağlı Görevler · İlerleme</div>';
      h += '<div class="ipv2-task-list">';
      linkedTasks.forEach(function (t) {
        // Bu görevin substepleri varsa onların evidence durumunu say; yoksa
        // node-level evidence toplamını kullan.
        var subs = Array.isArray(t.substeps) ? t.substeps : [];
        var subTotal = subs.length;
        var subDone = subTotal ? subs.filter(function (s) { return !!evState[s.id]; }).length : 0;
        var fallbackTotal = requiredEvs.length;
        var fallbackDone = doneCount;
        var dCount = subTotal ? subDone : fallbackDone;
        var dTotal = subTotal ? subTotal : fallbackTotal;

        var explicitlyDone = false;
        try {
          explicitlyDone = !!(window.App && Array.isArray(App.completedTasks) && App.completedTasks.indexOf(t.id) >= 0);
        } catch (e) {}
        var allDone = dTotal > 0 && dCount >= dTotal;
        var done = explicitlyDone || allDone;
        var partial = !done && dCount > 0;
        var critical = !!t.critical;

        var itemCls = 'task-item' + (done ? ' done' : (partial ? ' pending' : (critical ? ' pending critical-pending' : ' pending')));
        var mark = done ? '✓' : (partial ? '•' : (critical ? '!' : '•'));
        var checkCls = done ? 'state-done' : (partial ? 'state-pending' : (critical ? 'state-critical' : 'state-pending'));
        var statusLbl = done ? 'Tamamlandı' : (partial ? 'İlerliyor' : 'Bekliyor');
        var statusBadge = done ? 'state-done' : (partial ? 'free' : 'state-critical');

        h += '<div class="' + itemCls + '" style="cursor:default" title="Bu görev karta bağlı; ilerleme sağ panel ile senkron.">';
        h += '<div class="check ' + checkCls + '" aria-hidden="true">' + mark + '</div>';
        h += '<div class="task-main">';
        h += '<div class="task-head">';
        h += '<div class="lbl">' + esc(t.taskTitle || t.label || t.id) + '</div>';
        h += '<div class="badges">';
        if (critical) h += '<span class="badge crit">Kritik</span>';
        h += '<span class="badge ' + statusBadge + '">' + statusLbl + '</span>';
        if (dTotal) h += '<span class="badge gline">' + dCount + '/' + dTotal + ' adım</span>';
        h += '</div></div></div>';
        h += '</div>';
      });
      h += '</div></div>';
    ...
    // Substep Kontrol Listesi (Klinik Not yerine — yeşil-fill mekanikli)
    // Bağlı görevin substep dizisi gösterilir; node evidence ilerlemesine
    // orantılı olarak ilk N satır ✓ yeşil, kalan ○ gri kalır.
    var primaryTask = linkedTasks[0];
    var subs = primaryTask && Array.isArray(primaryTask.substeps) ? primaryTask.substeps : [];
    if (subs.length) {
      var stepDoneCount = 0;
      var taskCompleted = false;
      try {
        taskCompleted = !!(window.App && Array.isArray(App.completedTasks) && App.completedTasks.indexOf(primaryTask.id) >= 0);
      } catch (e) {}
      if (taskCompleted) {
        stepDoneCount = subs.length;
      } else {
        var totalEv = requiredEvs.length || subs.length;
        if (totalEv) {
          var ratio = doneCount / totalEv;
          stepDoneCount = Math.round(ratio * subs.length);
          if (doneCount > 0 && stepDoneCount === 0) stepDoneCount = 1;
        }
      }

      h += '<div class="ipv2-sec"><div class="ipv2-sec-t">Kontrol Listesi</div>';
      h += '<div class="ipv2-step-list">';
      subs.forEach(function (s, i) {
        var label = (typeof s === 'string') ? s : ((s && (s.label || s.text || s.title || s.id)) || '—');
        var stepDone = i < stepDoneCount;
        h += '<div class="ipv2-step' + (stepDone ? ' done' : '') + '">';
        h += '<span class="ipv2-step-icon">' + (stepDone ? '✓' : '○') + '</span>';
        h += '<span class="ipv2-step-text">' + esc(label) + '</span>';
        h += '</div>';
      });
      h += '</div></div>';
    }

    // GCKL Haritalama (kompakt etiket-değer ızgarası, preop dil paritesi)
    if (card.gcklMapping) {
    ...
    // İnline width/maxWidth set ETME — CSS .ipv2-host kuralı (380px) hallediyor.
    // (Eski 520px inline kontrolu kaldırıldı; preop popup'a geçişte de bu inline
    //  stiller sızıyordu — clearIntraopV2PopupState bunları zaten temizliyor.)
    host.classList.add('visible');
    try { host.setAttribute('data-intra-popups-v2', '1'); } catch (e) {}

    // Konumlandırma: preop showObjPopup paritesi — ortak helper kullanılır.
    // x/y null geldiğinde (re-render) lastAnchor üzerinden popup aynı yerde kalır.
    var useX = (typeof x === 'number') ? x : lastAnchor.x;
    var useY = (typeof y === 'number') ? y : lastAnchor.y;
    if (typeof useX === 'number' && typeof useY === 'number') {
      if (typeof window.positionPopupNearPoint === 'function') {
        window.positionPopupNearPoint(host, useX, useY, 380);
      } else {
        // Fallback: scene-shell yoksa veya helper yüklenmemişse basit clamp.
        var w = Math.min(390, window.innerWidth - 32);
        var px = Math.min(Math.max(10, useX + 14), window.innerWidth - w - 10);
        var py = Math.min(Math.max(10, useY + 14), window.innerHeight - 510);
        host.style.left = px + 'px';
        host.style.top = py + 'px';
      }
      // Anchor'u sakla — bir sonraki re-render aynı konumu kullanır
      if (typeof x === 'number' && typeof y === 'number') {
        lastAnchor.x = x;
        lastAnchor.y = y;
        lastAnchor.clinicalKey = clinicalKey;
      }
    }
    ...
    host.onclick = function (ev) {
      ...
      if (t.classList && (t.classList.contains('oclose') || t.classList.contains('ipv2-cls'))) {
        ...
        // v2 state temizliği — bir sonraki preop popup'a kalıntı sızmasın
        if (typeof window.clearIntraopV2PopupState === 'function') {
          window.clearIntraopV2PopupState(host);
        } else {
        ...
        // Anchor sıfırla — yeni popup yeni tıklama noktası kullansın
        lastAnchor.x = null; lastAnchor.y = null; lastAnchor.clinicalKey = null;
        return;
      }
      ...
      if (btn.dataset.ev && net.markEvidence) {
        ...
        // Re-render — lastAnchor sayesinde popup aynı konumda kalır
        renderIntraProtocolCard(card, clinicalKey, obj, null, null);
        return;
      }

      if (btn.dataset.mcq != null && net.answerRationale) {
        // (Eski inline yol — artık MCQ modal'da; bu dal yine de güvenli kalsın.)
      ...
      if (btn.dataset.mcqToggle) {
        // Mikro senaryoyu AYRI modal'da aç — ana kart boyutunu büyütme.
        openIntraopMcqModal(card, clinicalKey, obj, nodeId);
        return;
      }
