...
(function() {
    ...
    // BUG FIX (m0121): MSV onayı sonrası talk gate'i bypass eden tamamlama yardımcısı.
    // executeTaskCompletion'ı taklit eder ama talk gate'e takılmaz çünkü MSV'nin
    // 3-4 kaynaktan doğrulanmış onayı zaten kritik konuşma kanıtı içerir.
    function forceCompleteAfterMSV(taskId, sourceObj) {
        if (!window.App || !App.currentPatient) return false;
        if (Array.isArray(App.completedTasks) && App.completedTasks.includes(taskId)) {
            // Zaten tamam — sadece UI'ı tazele
            try { renderRightPanel(); } catch(e) {}
            return true;
        }
        const phase = App.currentPatient[App.currentRoom];
        if (!phase || !Array.isArray(phase.tasks)) return false;
        const t = phase.tasks.find(x => x && x.id === taskId);
        if (!t) {
            console.warn('[İP-3] forceCompleteAfterMSV: task bulunamadı', taskId);
            return false;
        }
        ...
        // v6 SM ile senkronize
        try { if (window.NurseKitSM?.shadowComplete) NurseKitSM.shadowComplete(taskId, sourceObj); } catch(e) {}

        // 3D marker yeşil pulse (varsa)
        try { if (sourceObj && typeof getMarkerCompletionKeyFromObject === 'function') {
            const mk = getMarkerCompletionKeyFromObject(sourceObj);
            if (mk && typeof completeMarkerBySourceKey === 'function') completeMarkerBySourceKey(mk, false);
        }} catch(e) {}
        try { if (typeof pulseMarkerComplete === 'function') pulseMarkerComplete(taskId, null); } catch(e) {}
        try { if (typeof gcklBoardSyncMarkerState === 'function') gcklBoardSyncMarkerState(); } catch(e) {}

        // Puan
        try {
            const earned = t.score || 0;
            if (typeof addScore === 'function') addScore(t.categories || [], earned, 0);
            if (typeof addPhaseScore === 'function') addPhaseScore(App.currentRoom, earned, 0);
        } catch(e) {}

        // Feedback + toast + OSCE
        try {
            if (Array.isArray(App.feedbackEntries)) App.feedbackEntries.push({
                ok: true, label: t.label, kind: 'task-msv',
                guideline: t.guideline, phase: App.currentRoom
            });
        } catch(e) {}
        try { if (typeof autoScoreOSCEFromTask === 'function') autoScoreOSCEFromTask(taskId, true, { label:t.label, phase:App.currentRoom }); } catch(e) {}
        try { if (typeof toast === 'function') toast('success', '✓ Görev Tamamlandı', t.label + ' · Çoklu kaynak doğrulaması tamamlandı'); } catch(e) {}
    ...
    function onMSVConfirm() {
        ...
        if (result.status === 'completed') {
            ...
            // BUG FIX (m0121): Original completeTask'taki talk-gate ('identitySite' vb.)
            // multi-source verification ZATEN sözel doğrulama içerdiği için
            // MSV onayında talk gate'i bypass etmemiz gerekiyor. Aksi halde
            // task.completedTasks.push hiç çalışmıyor → UI yeşile dönmüyor.
            //
            // Strateji: __nk_msv_orig_completeTask'ı çağırmak yerine
            // doğrudan executeTaskCompletion benzeri tam tamamlama yap.
            forceCompleteAfterMSV(taskId, sourceObj);
            return;
        }
        ...
        function renderPreopMapPanel(){
            ...
            sections.forEach(sec => {
                ...
                MAP.filter(m => m.section === sec).forEach(m => {
                    ...
                    row.onclick = () => {
                        try {
                            // m0176: Tıklayınca ilgili kart popup'ı açılır (önceden sadece toast vardı)
                            if (typeof showObjPopup === 'function' && m.cardKey) {
                                showObjPopup({ opts: { clinicalKey: m.cardKey }, clinicalKey: m.cardKey }, 0, 0);
                            }
                            // Ek olarak sahnede ilgili objeye dikkat çek (eski toast davranışı korunur)
                            showSceneReaction?.(`${m.title}: ${m.reasoning}`, done ? 'ok' : 'info');
                        } catch(e) {}
                    };
